...
#define RouteArgs                                                                                                                     \
  jsoncons::ojson const &valin, unsigned int eventNo, std::string const &routeNo, Session const &session, jsoncons::ojson &resultMsg, \
      ws_connector_actor_int::stateful_pointer<ok::smart_actor::connection::ws_controller_state> currentActor, std::string const &subDomain
namespace impl
{
void addAuthRoutes()
{
    ...
    routeFunctions["is_logged_in"] = [](RouteArgs)
    {
      ...
      if (!session.memberKey.empty() && utils::time::getEpochMilliseconds() < session.sessionEpoch + milliseconds) { ok::smart_actor::connection::addSuccess(resultMsg, event); }
      else { ok::smart_actor::connection::addFailure(resultMsg, event, "error", "not logged in"); }
    };
