...
#ifndef JSONCONS_JSON_ENCODER2_HPP
#define JSONCONS_JSON_ENCODER2_HPP
...
class basic_json_encoder2 final : public basic_json_visitor<CharT>
{
  ...
  private:
    ...
    basic_json_encoder2(const basic_json_encoder2&) = delete;
    basic_json_encoder2& operator=(const basic_json_encoder2&) = delete;
  public:
    basic_json_encoder2(Sink&& sink,
                        const Allocator& alloc = Allocator())
        : basic_json_encoder2(std::forward<Sink>(sink), basic_json_encode_options<CharT>(), alloc)
    {
    }
  ...
  private:
    ...
    bool visit_key(const string_view_type& name, const ser_context&, std::error_code&) override
    {
        ...
        //        sink_.push_back('\"');
        std::size_t length = jsoncons::detail2::escape_string(name.data(), name.length(),options_.escape_all_non_ascii(),options_.escape_solidus(),sink_);
        //        sink_.push_back('\"');
    ...
    bool visit_string(const string_view_type& sv, semantic_tag tag, const ser_context&, std::error_code&) override
    {
        ...
        switch (tag)
        {
            ...
            {
                sink_.push_back('\'');
                std::size_t length = jsoncons::detail2::escape_string(sv.data(), sv.length(),options_.escape_all_non_ascii(),options_.escape_solidus(),sink_);
                sink_.push_back('\'');
                column_ += (length+2);
                break;
            }
...
template<class CharT,class Sink=jsoncons::stream_sink<CharT>,class Allocator=std::allocator<char>>
class basic_compact_json_encoder2 final : public basic_json_visitor<CharT>
{
  ...
  private:
    ...
    int nesting_depth_;
            ...
            // Noncopyable
    basic_compact_json_encoder2(const basic_compact_json_encoder2&) = delete;
    basic_compact_json_encoder2& operator=(const basic_compact_json_encoder2&) = delete;
  public:
    basic_compact_json_encoder2(Sink&& sink,
                                const Allocator& alloc = Allocator())
        : basic_compact_json_encoder2(std::forward<Sink>(sink), basic_json_encode_options<CharT>(), alloc)
    {
    }
    ...
    basic_compact_json_encoder2(basic_compact_json_encoder2&&) = default;
    basic_compact_json_encoder2& operator=(basic_compact_json_encoder2&&) = default;

    ~basic_compact_json_encoder2() noexcept
    {
        JSONCONS_TRY
        {
            sink_.flush();
  ...
  private:
    ...
    bool visit_key(const string_view_type& name, const ser_context&, std::error_code&) override
    {
        ...
        //        sink_.push_back('\"');
        jsoncons::detail2::escape_string(name.data(), name.length(),options_.escape_all_non_ascii(),options_.escape_solidus(),sink_);
        //        sink_.push_back('\"');
        sink_.push_back(':');
        return true;
    }
    ...
    bool visit_string(const string_view_type& sv, semantic_tag tag, const ser_context&, std::error_code&) override
    {
        ...
        switch (tag)
        {
            ...
            {
                sink_.push_back('\'');
                jsoncons::detail2::escape_string(sv.data(), sv.length(),options_.escape_all_non_ascii(),options_.escape_solidus(),sink_);
                sink_.push_back('\'');
                break;
            }
        }
