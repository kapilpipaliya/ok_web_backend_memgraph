...
brew install pkg-config autoconf automake git make cmake gdb  libffi zlib openssl gsasl gnutls zip unzip
...
./vcpkg install drogon caf jsoncons date gtest cpp-jwt parallel-hashmap tsl-ordered-map --debug

# Navigate back to the project directory
cd $project_dir

# Copy json_encoder2.hpp to the appropriate directory
cp $project_dir/third_party/one_file_lib/json_encoder2.hpp $vcpkg_dir/installed/x64-osx/include/jsoncons/json_encoder2.hpp
...
cmake --build $server_agent_build_dir -- -j$(nproc)
